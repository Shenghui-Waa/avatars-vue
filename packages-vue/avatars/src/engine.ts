...
/**
 * Stable string to 32-bit unsigned hash (FNV-1a + bit-mixing avalanche).
...
/** `#rgb`/`#rrggbb` (with or without `#`) to `#RRGGBB`, or null if invalid. */
 ...
 * displayed canvas (about 6% of the rendered dimension) for the signature look,
...
/* -- dither: ordered (Bayer 8x8) ramp of the palette along a random axis -- */
...
const BAYER = makeBayer(3); // 8x8, thresholds in (0,1)

/**
 * Draw an ordered (Bayer 8x8) dither of the seed's palette into `ctx` at
